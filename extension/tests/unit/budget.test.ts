...
import { checkBudget, clampNumCtx, budgetsFor, capsFor, DEFAULT_NUM_CTX } from '@/agent/budget';
...
describe('checkBudget', () => {
  ...
  it('marks executor as needing compaction at 80%', () => {
    const big = 'x'.repeat(budgetsFor(DEFAULT_NUM_CTX).executor * 4 * 0.85);
  ...
  it('does not request compaction for non-executor roles', () => {
    const big = 'x'.repeat(budgetsFor(DEFAULT_NUM_CTX).planner * 4 * 0.85);
    const r = checkBudget('planner', big, est);
    expect(r.shouldCompact).toBe(false);
  });

  it('flags overBudget when exceeding limit', () => {
    const huge = 'y'.repeat(budgetsFor(DEFAULT_NUM_CTX).executor * 4 * 2);
