...
export const SEED_WORKFLOWS: Workflow[] = [
  ...
  },
  {
    ...
    goalSample: 'apply to a job: fill the application form from my profile and submit it',
    steps: [
      ...
      { instruction: 'Fill each field by typing the matching value from USER PROFILE (name, email, phone, etc.). Skip the resume file upload — not supported.', toolHint: 'tab.type' },
      { instruction: 'Re-read the form to confirm the fields are filled (auto-refreshed after typing).', toolHint: 'aria.extract' },
      { instruction: 'Submit the application.', toolHint: 'tab.click (submit) or tab.type submit:true' },
      { instruction: 'Read the confirmation and report whether the application was submitted.', toolHint: 'finish' },
    ],
  },
];
...
export function traceToWorkflow(
  ...
  for (const t of trace) {
    ...
    if (prev && prev.toolHint === step.toolHint) continue; // collapse consecutive duplicates
    steps.push(step);
  }
