...
For PRICE-SENSITIVE goals (flights, hotels, rentals, event tickets, products where price changes often), search-result SNIPPET prices are NEVER sufficient — they are generic "starting from" teasers, not actual live prices for the user's specific dates/requirements. Steps that look up prices MUST require opening and reading an actual booking/listing page with the specific parameters filled in, and the successCriteria MUST state that real prices for the specific dates/items are displayed on that page, not in search snippets. Similarly, when the goal has a DATE component ("upcoming long weekend", "next month", "this weekend"), the FIRST step must determine the actual calendar dates (search for "US federal holidays 2026" or "current date") before any price search — do NOT guess dates or use vague terms like "this weekend" in a flight search query.
...
- After clicking a Search/Submit button on a flight, hotel, or booking site, the results page often still shows parts of the search form or a date picker overlay — the actual results (prices, airlines, times) are BELOW it. Scroll down (tab.scroll down) or look past the picker instead of assuming the search failed. If a date picker dialog is open and blocking results, look for a "Done" or "Close" button and click it — do NOT hunt for invisible month-navigation arrows with vision.read.
...
- Flight/hotel/booking sites: after Search is clicked, the results page often still shows a date picker or search form overlay on top of the actual results (prices, airlines, times). If the URL has changed and page content is substantial (not an empty/error page), do NOT FAIL just because a date picker dialog is visible — the results are loaded underneath. Judge the page as PASS if flight listings/prices are present in the page content, even if a picker overlay is also visible.
