 ...
 *  Always opens a fresh tab — the old dedup check (reuse if same URL) was silently
 *  collapsing the agent's tabs, making it impossible to find a previously opened
 *  page when the same URL appeared in a later search result. The agent can close
 *  tabs it no longer needs via tab.close, and the orchestrator cleans up all owned
 *  tabs on finish/abort. */
