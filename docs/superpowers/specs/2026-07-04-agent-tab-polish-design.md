...
**Corrected after implementation review** (see `docs/superpowers/plans/2026-07-04-agent-tab-polish.md`, Task 3, for the full account): the version below replaces an earlier draft of this section that specified a fixed-height flex shell (`.app` at `height:100%` with an inner `flex:1; overflow-y:auto` scroll region). That pattern was implemented, passed typecheck and the full test suite, and only then was found — by reading `styles.css`'s own git history — to be the exact pattern this codebase had already tried three times (commits `c793cae`, `20f77dc`, `05f17dd`, `d651bf0`) and abandoned in `d0c77d1` because Chrome side panels don't reliably resolve percentage-height chains, a failure mode that only manifests in a real loaded panel, never in automated checks. Corrected before merging.

The Agent tab's markup stays a single flat flex column — the panel keeps scrolling as a normal document (unchanged: `min-height: 100%` on `html`/`body`/`#root`, no fixed height or `overflow:hidden` on `.app`, no inner scroll container). Two elements use `position: sticky` to stay visually anchored while the document scrolls around them — the same technique `.save-bar` already uses successfully today in `SettingsPanel` (`position: sticky; bottom: calc(-1 * var(--sp-4));`):

1. **Sticky header** — `ConnectionCard` (if Ollama is down) + `SessionSwitcher`, `position: sticky; top: 0`. Stays visible once scrolled past, so the active chat's identity doesn't disappear during a long scroll.
2. **Normal-flow middle** — the `notice` `Alert`, `Transcript` (past turns), the active turn's `RunState`/`ResultCard`/`Timeline`, and the empty state — flat children of the same flex column, no special scroll handling needed since the whole document scrolls.
3. **Sticky footer** — `Composer`, `position: sticky; bottom: calc(-1 * var(--sp-4))` (the same negative-offset-equal-to-padding trick `.save-bar` already uses, needed because the sticky containing block's own bottom padding would otherwise leave a gap).

**Auto-scroll to latest**: `window.scrollTo({ top: document.documentElement.scrollHeight })` on the same triggers (new turn, streamed event, turn finishes) — targets the document's own scroll position directly rather than an inner ref, consistent with there being no inner scroll container.

**Accepted trade-off**: `position: sticky` keeps the composer visible once you'd scroll past it, but — unlike a height-pinned flex footer — doesn't force it to the absolute bottom edge of an under-filled viewport (e.g. a brand-new empty chat has nothing to scroll, so the composer just sits below the empty-state card rather than glued to the panel's bottom edge). This is the accepted cost of using the one pattern this codebase has field-verified to actually work in a real Chrome side panel.
...
**Two refinements found during implementation review, not anticipated by the design above:**

1. `Brand` renders its own root as `.app-header` (`position: sticky; top: 0; z-index: 20`) — unconditionally, on every tab. Since `.agent-tab-header` also sticks at `top: 0`, the two would occupy the identical coordinate once scrolled past the (non-sticky) `Tabs` bar, and Brand's higher z-index would permanently cover `SessionSwitcher` for the rest of the scroll. Fixed by measuring `Brand`'s actual rendered height at runtime (`ResizeObserver` on a wrapping ref, not a hardcoded pixel guess — robust to font-size/content changes) and applying it as `.agent-tab-header`'s `top` via an inline style that overrides the CSS class's `top: 0` fallback.
2. The auto-scroll effect originally fired on every `events` update regardless of which tab was showing, and wouldn't catch up immediately when switching back to the Agent tab mid-run. Fixed by gating the effect on `tab === 'agent'` and adding both `tab` and `status` (the latter because `RunState`'s plan checklist can advance via a `status` SW message with no coincident `events` append) to its dependency array.
...
Change the Planner's contract: its prompt gains an explicit instruction that when GOAL doesn't describe an actionable task, it should respond with exactly `{"noGoal":true}` instead of inventing a step that just asks for a goal. **Implementation note (superseding the paragraph as originally drafted):** this ships as a distinct `noGoal?: boolean` field on the plan JSON, not an empty `steps: []` array — an empty-steps response is already a pre-existing, differently-handled failure mode (the "small model emitted a malformed/wrong-shaped plan" retry), and overloading it to also mean "deliberately no goal" would make the two cases indistinguishable. A dedicated field disambiguates cleanly: `noGoal:true` skips straight to `blocked`, while a genuinely malformed/empty response still gets the existing one-shot retry before failing. In `orchestrator.ts`, immediately after the Planner call returns (in `plan()`, before the resulting `Plan` is applied and before entering the Executor/Evaluator loop at all), check for `noGoal` and short-circuit straight to a `blocked` finish — mirroring the existing `finishOk`/`abortNow` pattern, with a summary like "I need a clearer goal to work with — could you tell me what you'd like me to do?" This is the same "skip machinery that can't help" idea `agent.askPage`'s seeded-plan already uses, applied at the other end (skip straight to finish, instead of skip straight to a pre-built plan).
...
**Scope check on empty-plan safety**: `plan.ts`'s `newPlan`/`currentStep`/`walkPlan` and the rest of the run loop have never had to handle a zero-step plan before (every existing code path assumes at least one step). Rather than making that machinery newly tolerant of an empty plan, the short-circuit happens BEFORE a `Plan` is ever constructed or handed to `applyPlan`/`currentStep` — a throwaway `newPlan([])` is returned alongside `noGoal:true` but is never applied — so nothing downstream needs to change or be made empty-plan-safe. The implementation plan should verify the exact call site in `plan()`/`runUntilTerminal()` where this check cleanly intercepts before any current-step-dependent code runs.
