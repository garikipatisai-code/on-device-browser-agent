...
- **Grounded memory.** As it works it keeps a compact **facts ledger** — the key value each step established, copied verbatim from the page — and carries it into every later step, so long multi-step tasks don't forget what they found early. A **workspace** auto-saves structured notes from every page visited and injects them into every turn's context; you can also add, list, and clear notes manually. Final answers are **verified against what was actually read**: a number that never appeared on a page is flagged, not asserted.
- **Acts:** open tabs, search the web, open a result, click elements, type into fields (single or batched), submit forms, scroll, attach a file to an upload field. **Page-level tools** for SPAs and JS-heavy sites: query the rendered DOM by CSS selector, wait for elements to appear, subscribe to real-time DOM mutations, fetch REST APIs directly (CORS-free from the extension context), and run deterministic JS postcondition checks. Plus **vision.verify** to check visual state with a screenshot + multimodal model.
...
- **Chat-style sessions.** Past runs are saved as sessions you can switch between, like chat history — start a new one, revisit an old one, or delete one; each keeps its own goal/answer trail.
...
- **Pauses on anti-bot walls, never solves them.** If a page shows a CAPTCHA, a Cloudflare/Akamai-style "checking your browser" interstitial, or a generic bot-block, the agent pauses and waits for you to resolve it in the tab, then continues automatically once it clears — it never attempts to solve, click through, or bypass one.
...
- **Optional: hybrid mode.** Let the planner and evaluator run on a frontier model instead of the local one — Anthropic directly, or any backend speaking the OpenAI Chat Completions API (OpenAI, OpenRouter, DeepSeek, MiniMax, a self-hosted server — just point it at a base URL). Everything that actually touches the browser — reading pages, clicking, typing, uploading — always stays local and on-device by default. An additional `hybridHelpers` toggle can extend frontier to the helper seat too, but it's off by default and gated behind hybrid mode. Default **off**; with it off, behavior is unchanged from fully local. Turning it on means page content the evaluator judges can reach that third-party API — an explicit trade you opt into, not a silent one. Configure it in **Settings → Frontier model (optional)**, with one-click presets for common provider/model combos.
